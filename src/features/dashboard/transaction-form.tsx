      ...
      <div className="flex md:hidden overflow-x-auto pb-4 gap-2 snap-x scrollbar-hide -mx-2 px-2 mb-2">
         {allTemplates.map((template) => (
           <button 
            ...
            onClick={() => applyTemplate(template)}
            disabled={isLoading}
            className="snap-start shrink-0 px-4 py-3 border border-white/5 rounded-2xl bg-secondary hover:bg-[#222222] active:scale-95 transition-all text-xs font-medium text-muted-foreground cursor-pointer flex items-center gap-1.5 disabled:opacity-50"
           >
              <span>{template.category?.icon || "⚡"}</span>
              <span>{template.title}</span>
           </button>
         ...
         <button 
          onClick={() => router.push('/settings?tab=shortcuts')}
          disabled={isLoading}
          className="snap-start shrink-0 px-4 py-3 border border-white/5 rounded-2xl bg-secondary/50 text-xs font-medium text-muted-foreground/60 cursor-pointer disabled:opacity-50"
         >
          + Thêm
         </button>
      </div>
