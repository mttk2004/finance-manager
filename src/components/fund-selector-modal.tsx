    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
      <div className="bg-card border border-border rounded-3xl p-6 max-w-sm w-full shadow-2xl relative">
        ...
        <div className="space-y-2 mb-6">
          {funds.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-4">Chưa có quỹ nào</p>
          ) : (
            funds.map(fund => (
              <button 
                key={fund.id}
                ...
                className={`w-full flex items-center justify-between p-4 rounded-xl border transition-all cursor-pointer ${
                  currentFund === fund.name 
                    ...
                    : 'bg-secondary border-white/[0.02] hover:bg-[#222222]'
                }`}
              >
                <span className={`font-medium ${currentFund === fund.name ? 'text-emerald-400' : 'text-neutral-300'}`}>
                  {fund.name}
                </span>
                <span className={`font-mono text-sm ${currentFund === fund.name ? 'text-emerald-500/80' : 'text-muted-foreground'}`}>
        ...
        <button 
          ...
          className="w-full py-3 rounded-xl border border-border border-dashed text-muted-foreground font-medium text-sm hover:bg-white/[0.02] hover:text-white transition-colors flex items-center justify-center gap-2 cursor-pointer"
        >
